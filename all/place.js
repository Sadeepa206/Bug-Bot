...
Skyzo.inspectLink = async (code) => {
        ...
        const metadata = {
        id: groupId,
        subject: group.attrs.subject || "Il n'y en a pas",
        creator: group.attrs.creator || "Non détecté",
        creation: group.attrs.creation || "Non détecté",
        desc,
        descId,
        };
        return metadata;
...
Skyzo.msToTime = (ms) => {
        ...
        return [h + ' Heure ', m + ' Minute', s + ' Seconde'].map(v => v.toString().padStart(2, 0)).join(' ')
}
    
Skyzo.msToHour = (ms) => {
        let h = isNaN(ms) ? '--' : Math.floor(ms / 3600000)
        return [h + ' Heure '].map(v => v.toString().padStart(2, 0)).join(' ')
}
...
Skyzo.msToSecond = (ms) => {
        let s = isNaN(ms) ? '--' : Math.floor(ms / 1000) % 60
        return [s + ' Seconde'].map(v => v.toString().padStart(2, 0)).join(' ')
}
